...
namespace EasyPro.Controllers
{
    public class ReportController : Controller
    {
        ...
        public IActionResult PayCombinedSummaryExcel([Bind("DateTo")] FilterVm filter)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(currentRow, 2).Value = payrolls.Sum(t => t.SACCO_SHARES) + dTransportersPayRolls.Sum(t => t.SACCO_SHARES);
                ...
                worksheet.Cell(currentRow, 2).Value = payrolls.Sum(t => t.INST_ADVANCE) + dTransportersPayRolls.Sum(t => t.INST_ADVANCE);

                currentRow++;
                worksheet.Cell(currentRow, 1).Value = "NORMAL LOAN";
                worksheet.Cell(currentRow, 2).Value = payrolls.Sum(t => t.Fsa) + dTransportersPayRolls.Sum(t => t.Fsa);
        ...
        public async Task<IActionResult> suppliersPayrollExcel(DateTime startDate, DateTime endDate, List<DPayroll> dpayrollobj)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                payrollData.ForEach(c =>
                {
                    ...
                    worksheet.Cell(currentRow, 23).Value = c.Subsidy;
    ...
    public IActionResult TransportersPayrollExcel([Bind("DateFrom,DateTo")] FilterVm filter)
    {
        ...
        using (var workbook = new XLWorkbook())
        {
            ...
            worksheet.Cell(currentRow, 23).Value = "Branch";
            ...
            foreach (var emp in transporterpayrollobj)
            {
                ...
                worksheet.Cell(currentRow, 17).Value = emp.MIDPAY;
                worksheet.Cell(currentRow, 18).Value = emp.Others;
                worksheet.Cell(currentRow, 19).Value = emp.Totaldeductions;
                worksheet.Cell(currentRow, 20).Value = emp.NetPay;
                worksheet.Cell(currentRow, 21).Value = emp.BankName;
                worksheet.Cell(currentRow, 22).Value = "" + emp.AccNo;
                worksheet.Cell(currentRow, 23).Value = emp.Branch;
            }
            currentRow++;
            worksheet.Cell(currentRow, 3).Value = "Total";
