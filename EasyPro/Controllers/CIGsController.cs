...
namespace EasyPro.Controllers
{
    public class CIGsController : Controller
    {
        ...
        public CIGsController(MORINGAContext context, INotyfService notyf, IReportProvider reportProvider)
        {
            _context = context;
            _notyf = notyf;
        ...
        public async Task<IActionResult> Details(long? id)
        {
            ...
            var productCat = await _context.CIGs
        ...
        public async Task<IActionResult> Create([Bind("Id,Name")] CIGs cIGs)
        {
            var loggedInUser = HttpContext.Session.GetString(StrValues.LoggedInUser) ?? "";
            if (string.IsNullOrEmpty(loggedInUser))
                return Redirect("~/");
            utilities.SetUpPrivileges(this);
            var sacco = HttpContext.Session.GetString(StrValues.UserSacco) ?? "";
            var extist = _context.CIGs.Any(c => c.Name.ToUpper().Equals(cIGs.Name.ToUpper())
            && c.saccocode == sacco);
            if (extist)
            {
        ...
        public async Task<IActionResult> Edit(long? id)
        {
            ...
            var productCat = await _context.CIGs.FindAsync(id);
            if (productCat == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(long id, [Bind("Id,Name")] CIGs productCat)
        {
            var loggedInUser = HttpContext.Session.GetString(StrValues.LoggedInUser) ?? "";
            if (string.IsNullOrEmpty(loggedInUser))
                return Redirect("~/");
            ...
            var sacco = HttpContext.Session.GetString(StrValues.UserSacco) ?? "";
            var extist = _context.CIGs.Any(c => c.Name.ToUpper().Equals(productCat.Name.ToUpper())
        ...
        public async Task<IActionResult> DeleteConfirmed(long id)
        {
            ...
            utilities.SetUpPrivileges(this);
            var productCat = await _context.CIGs.FindAsync(id);
            _context.CIGs.Remove(productCat);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            return _context.CIGs.Any(e => e.Id == id);
        }
    }
}
